...
CatArrEqIsEquiv c = (x, y : Obj c) -> IsEquivRel (IHom c x y) (IArrowEq c x y) -- (IArrowEq' c x y) -- (===)
...
FunctorIdIsaFunctor c ceqax = MkFunctorLaws 
                          (\_, _, f, g, pfgeq => pfgeq)                    -- f === g -> f === g
                          (\x => (Eq_refl (ceqax x x) (id x)))             -- id x === id x
                          (\_,_,_, g, f => (Eq_refl (ceqax _ _) (g . f)))  -- g . f === g . f
